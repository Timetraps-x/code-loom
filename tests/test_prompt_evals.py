...
def test_loom_do_skill_prompt_eval_bad_cases():
    ...
    cases = (
        ...
        PromptEvalCase(
            name="skill_requires_internal_seal_changes_after_modification",
            surface=prompt,
            badcase="builder modifies files and host invokes reviewer on full working tree diff or asks the user to seal changes manually",
            required_guardrails=("internal runtime action that seals the change set", "automatically run", "do not ask the user to run it", "seal-changes"),
        ),
        PromptEvalCase(
        ...
        PromptEvalCase(
            name="skill_requires_latest_seal_for_implemented",
            surface=prompt,
            badcase="host completes implemented after builder changes files post-review",
            required_guardrails=("automatically rerun `action=seal-changes`", "seal_revision", "review_status=pass"),
        ),
    )
...
def test_loom_do_skill_prompt_eval_host_handoff_cases():
    ...
    for expected in (
        ...
        "internal runtime action that seals the change set",
        "do not present it as a user-facing stage",
        ...
        "host-provided attempt-scoped diff",
        "seal_revision",
        "automatically include latest sealed changes and review result",
        "review_status=pass",
        "action=complete",
